class ArticlesController < ApplicationController
  ...
  def authorize_user
    unless Article.find_by_id(params[:id]).user_id == session[:user_id] || User.find_by_id(session[:user_id]).admin == true
  ...
  def create
    ...
    if @article.save
     redirect_to article_url(@article), notice: "Article successfully added"
    else
      render 'new'
    end
  end
